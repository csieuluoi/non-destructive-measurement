for var = 1:3
    image_real = double(imread(['45D_500kHz_' num2str(2^var*100) 'u_realpart.tif']));
    image_imag = double(imread(['45D_500kHz_' num2str(2^var*100) 'u_imagpart.tif']));
...
%     image_imag = double(buf_imag);
    ...
    figure()
    mesh(image_real);
    title(['45D 500kHz ' num2str(2^var*100) 'µm Realpart']);
    
    figure()
    mesh(image_imag);
    title(['45D 500kHz ' num2str(2^var*100) 'µm Imagpart']);
    
    figure()
    mesh(abs(image_complex));
    title(['45D 500kHz ' num2str(2^var*100) 'µm Module']);
    
    figure()
    mesh(angle(image_complex));
    title(['45D 500kHz ' num2str(2^var*100) 'µm Phase']);
end
...
% Ma tran anh bo nen:
    ...
    figure()
    mesh(image_sub);
    title(['45D 500kHz ' num2str(2^var*100) 'µm -- Subtracted Module']);
...
%     end
    ...
    figure()
    mesh(length_finding)
    title(['45D 500kHz ' num2str(2^var*100) 'µm -- Length Finding']);
    ...
    disp(['45D 500kHz ' num2str(2^var*100) 'µm'])
