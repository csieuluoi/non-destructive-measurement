...
def find_contour(img):
    ...
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    print(gray.shape)
    ret, gray = cv2.threshold(gray, 0, 200, 0)
    ...
    ret, gray = cv2.threshold(roi_gray, 0,150,0)
    contours, hierarchy = cv2.findContours(gray, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    ...
    start_point, end_point = get_bbox(roi_cord, bbox_size = (20, 20))
...
def get_bbox(roi_cord, bbox_size = (30, 20)):
    ...
    new_max_x = new_min_x + bbox_size[0]
    new_max_y = new_min_y + bbox_size[1]
