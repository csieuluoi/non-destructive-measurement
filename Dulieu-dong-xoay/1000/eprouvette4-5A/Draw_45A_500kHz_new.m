for var = 1:3
    buf_real = imread(['45A_500kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['45A_500kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
for var = 1:1
    image_real = double(imread(['45A_500kHz_' num2str(2^var*100) 'u_realpart.tif']));
    image_imag = double(imread(['45A_500kHz_' num2str(2^var*100) 'u_imagpart.tif']));
...
% Finding the max value (module -real value) and its position:
    ...
    figure()
    mesh(image_module);
    title(['45A 500kHz ' num2str(2^var*100) 'µm -- Module']);
    
    figure()
    mesh(image_real);
    title(['45A 500kHz ' num2str(2^var*100) 'µm -- Real Part']);
    
    figure()
    mesh(image_imag);
    title(['45A 500kHz ' num2str(2^var*100) 'µm -- Imag Part']);
    %-----
...
%     end
    ...
    disp(['45A 500kHz ' num2str(2^var*100) 'µm'])
...
%     AVE_angle1 = mean(mean(angle_max_mat1));
    ...
    title(['45A ' num2str(2^var*100) 'µm']);
%     figure()
...
title('45A 1000 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('45A 1000 -- Angle max & Depth --');