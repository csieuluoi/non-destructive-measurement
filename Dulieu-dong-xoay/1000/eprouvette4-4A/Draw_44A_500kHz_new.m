for var = 1:3
    buf_real = imread(['44A_500kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['44A_500kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
for var = 1:1
    image_real = double(imread(['44A_500kHz_' num2str(2^var*100) 'u_realpart.tif']));
    image_imag = double(imread(['44A_500kHz_' num2str(2^var*100) 'u_imagpart.tif']));
...
% Finding the max value (module -real value) and its position:
    ...
    figure()
    mesh(image_module);
    title(['44A 500kHz ' num2str(2^var*100) 'µm -- Module']);
    %-----
...
%     end
    ...
    disp(['44A 500kHz ' num2str(2^var*100) 'µm'])
...
title('44A 1000 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('44A 1000 -- Angle max & Depth --');