for var = 1:3
    buf_real = imread(['2H_500kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['2H_500kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
for var = 1:1
    image_real = double(imread(['2H_500kHz_' num2str(2^var*100) 'u_realpart.tif']));
    image_imag = double(imread(['2H_500kHz_' num2str(2^var*100) 'u_imagpart.tif']));
...
line_bottom = line_top;
    ...
    for col = 4:image_size(2)-1
        ...
        for line = image_size(1)-12:image_size(1)-10    %24-7
...
% Finding the max value (module -real value) and its position:
    ...
    %---- Co-ordinates -------------------
    %---- for module image ---
    for vari = 1:im_corr_size(1)
        x_axe(vari) = vari;
    end
    
    for vari = 1:im_corr_size(2)
        y_axe(vari) = vari;
    end
    
    %---- for real and imag. parts ------
    im_size = size(image_real)
    for vari = 1:im_size(1)
        x_axe1(vari) = vari;
    end
    for vari = 1:im_size(2)
        y_axe1(vari) = vari;
    end
    ...
    figure(1)
%     mesh(image_module);
    surf(y_axe,x_axe,image_module);
%     mesh(y_axe,x_axe,image_module);
%     title(['2H 500kHz ' num2str(2^var*100) 'µm -- Module']);
    
     figure(2)
%     mesh(image_real);
    surf(y_axe1,x_axe1,image_real);
%     title(['2H 500kHz ' num2str(2^var*100) 'µm -- Real Part']);
    
     figure(3)
%     mesh(image_imag);
    surf(y_axe1,x_axe1,image_imag);
%     title(['2H 500kHz ' num2str(2^var*100) 'µm -- Imag Part']);
    %-----
...
%     end
    ...
    disp(['2H 500kHz ' num2str(2^var*100) 'µm'])
...
for var = 2:3
   ...
   figure()
   mesh(real(image_complex));
   title(['2H 500kHz ' num2str(2^var*100) 'µm -- Real part before noise reduce process']);
   
   figure()
   mesh(imag(image_complex));
   title(['2H 500kHz ' num2str(2^var*100) 'µm -- Imag part before noise reduce process']);
...
% Hieu chinh kich thuoc anh cu cho phu hop voi anh nen:
   ...
   figure()
   mesh(real(image_corr1));
   title(['2H 500kHz ' num2str(2^var*100) 'µm -- Real part after noise reduce process']);
   
   figure()
   mesh(imag(image_corr1));
   title(['2H 500kHz ' num2str(2^var*100) 'µm -- Imag part after noise reduce process']);
...
% Tim gia tri lon nhat so voi nen va toa do cua diem anh tuong ung:
    ...
    %---- Co-ordinates ----------------------------------------------
    %---- for image module ----
    for vari1 = 1:im_corr_size(1)
        x_axe2(vari1) = vari1;
    end
    for vari1 = 1:im_corr_size(2)
        y_axe2(vari1) = vari1;
    end
    ...
    for vari1 = 1:im_size(1)
        x_axe3(vari1) = vari1;
    end
    for vari1 = 1:im_size(2)
        y_axe3(vari1) = vari1;
    end
    ...
    mesh(image_module1);
%     title(['2H 500kHz ' num2str(2^var*100) 'µm -- Module']);
%     surf(y_axe2,x_axe2,image_module1);
%     mesh(y_axe2,x_axe2,image_module1);
    
    figure()
    mesh(image_real);
%     title(['2H 500kHz ' num2str(2^var*100) 'µm -- Real Part']);
%     surf(y_axe3,x_axe3,image_real);
%     mesh(y_axe3,x_axe3,image_real);
    
    figure()
    mesh(image_imag);
%     title(['2H 500kHz ' num2str(2^var*100) 'µm -- Imag part']);
%     surf(y_axe3,x_axe3,image_imag);
%     mesh(y_axe3,x_axe3,image_imag);
    %-----
...
title('2H 1000 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('2H 1000 -- Angle max & Depth --');