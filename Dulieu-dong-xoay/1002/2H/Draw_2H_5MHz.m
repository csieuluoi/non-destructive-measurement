...
%     image_imag = double(buf_imag);
    ...
    figure()
    mesh(image_real);
    title(['2H 5MHz ' num2str(2^var*100) 'µm Realpart']);
    
    figure()
    mesh(image_imag);
    title(['2H 5MHz ' num2str(2^var*100) 'µm Imagpart']);
    
    figure()
    mesh(abs(image_complex));
    title(['2H 5MHz ' num2str(2^var*100) 'µm Module']);
    
    figure()
    mesh(angle(image_complex));
    title(['2H 5MHz ' num2str(2^var*100) 'µm Phase']);
end