for var = 1:3
    buf_real = imread(['45D_400kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['45D_400kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
% Modified image_complex (fit the size of image_ground):
    ...
    figure()
    mesh(abs(image_corr));
    title(['45D 400kHz ' num2str(2^var*100) 'µm -- Module before subtraction']);
...
%     AVE_angle1 = mean(mean(angle_max_mat1));
    ...
    title(['45D ' num2str(2^var*100) 'µm']);
%     figure()
...
title('45D 1003 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('45D 1003 -- Angle max & Depth --');