...
def load_module(folder_name = 'module'):
	module_img_list = []
	cwd = os.getcwd()
	module_dir = os.path.join(cwd,'generated_imgs', folder_name)
...
def contour_cv2(module_img_list):
	...
	for img in module_img_list:
		# canny_output = cv2.Canny(img, 5, 40)
		...
		plt.subplot(142),plt.imshow(img),plt.title('Canny Binary Image')
		# plt.xticks([]), plt.yticks([])
...
def preparing_images(imgs, step = 15, lengths = ['200', '400', '750'], cut_img_size = None, padding_size = (90, 90), padding_type = 'edge', output_img_size = (20, 20), save = False, plot = False):
	...
	for i, (img, length) in enumerate(zip(padded_imgs, lengths)):
		...
		if save:
			save_rotate_img(imgs, fold_name = f'{step}_rotated_cut_imgs', length = length)
...
if __name__ == '__main__':
	...
	# index_cut = [[ (16, 46), (0, 20) ], [(7, 37), (20, 40)], [(7, 37), (20, 40)]]

	steps = [10, 15, 25, 29]
	lengths = ['200', '400', '1050']


	print([img.shape for img in normalized_image_list])
	padding_types = ['edge', 'linear_ramp', 'minimum']
	# prepare procedure: cut only the desire part  -> padding to get
	# using padding_type : 'minimum'
	for step in steps:
		preparing_images(normalized_image_list, step = step, lengths = lengths, cut_img_size = None,
			padding_size = (90, 90), padding_type = padding_types[1], output_img_size = (40, 40), save = True, plot = False	)


	"""trying to use contour to locate the hole"""
	# module_img_list = load_module('normalized_module')

	# imgs = rotate_img(module_img_list[1], angle_step = 90)
	# contour_cv2(imgs)
