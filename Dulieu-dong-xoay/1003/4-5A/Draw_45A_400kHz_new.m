for var = 1:3
    buf_real = imread(['45A_400kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['45A_400kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
% Modified image_complex (fit the size of image_ground):
    ...
    figure()
    mesh(abs(image_corr));
    title(['45A 400kHz ' num2str(2^var*100) 'µm -- Module before subtraction']);
...
title('45A 1003 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('45A 1003 -- Angle max & Depth --');