for var = 1:3
    buf_real = imread(['2A_400kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['2A_400kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
% Modified image_complex (fit the size of image_ground):
    ...
    figure()
    mesh(abs(image_corr));
    title(['2A 400kHz ' num2str(2^var*100) 'µm -- Module before subtraction']);
...
%     image_sub1 =  abs(image_ground1 - image_corr1);
    ...
    image_sub1 =  soustrait_fond(image_corr1); % --> complex
...
title('2A 1003 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('2A 1003 -- Angle max & Depth --');