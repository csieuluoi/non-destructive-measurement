for var = 1:3
    buf_real = imread(['44A_400kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['44A_400kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
% Modified image_complex (fit the size of image_ground):
    ...
    figure()
    mesh(abs(image_corr));
    title(['44A 400kHz ' num2str(2^var*100) 'µm -- Module before subtraction']);
...
%     image_sub1 =   soustrait_fond(image_corr1);
    ...
    title(['44A 500kHz ' num2str(2^var*100) 'µm -- Real']);
    
    figure()
    mesh(imag(image_sub1));
    title(['44A 500kHz ' num2str(2^var*100) 'µm -- Imag']);
...
%     AVE_angle1 = mean(mean(angle_max_mat1));
    ...
    title(['44A ' num2str(2^var*100) 'µm']);
%     figure()
...
title('44A 1003 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('44A 1003 -- Angle max & Depth --');