...
%     angle_max = image_phase(max_lin+10,max_col+3);
    ...
    for lin = max_lin-1:max_lin+1
        for col = max_col-1:max_col+1
            angle_max_mat(lin - (max_lin - 2),col - (max_col - 2)) = image_phase(lin,col);
        end
    end
...
for var = 2:3
   ...
   title(['2H 400kHz ' num2str(2^var*100) 'µm -- Real part before noise reduce process']);
   
   figure()
   mesh(imag(image_complex));
   title(['2H 400kHz ' num2str(2^var*100) 'µm -- Imag part before noise reduce process']);
...
% Hieu chinh kich thuoc anh cu cho phu hop voi anh nen:
   ...
   title(['2H 400kHz ' num2str(2^var*100) 'µm -- Real part after noise reduce process']);
   
   figure()
   mesh(imag(image_corr1));
   title(['2H 400kHz ' num2str(2^var*100) 'µm -- Imag part after noise reduce process']);
...
%     angle_max = image_phase(max_lin+3,max_col+19);
    ...
    for lin = max_lin-1:max_lin+1
        for col = max_col-1:max_col+1
            angle_max_mat1(lin - (max_lin - 2),col - (max_col - 2)) = image_phase(lin,col);
        end
    end
