for var = 1:3
    image_real = double(imread(['45A_400kHz_' num2str(2^var*100) 'u_realpart.tif']));
    image_imag = double(imread(['45A_400kHz_' num2str(2^var*100) 'u_imagpart.tif']));
...
%     image_imag = double(buf_imag);
    ...
    figure()
    mesh(image_real);
    title(['45A 400kHz ' num2str(2^var*100) 'µm Realpart']);
    
    figure()
    mesh(image_imag);
    title(['45A 400kHz ' num2str(2^var*100) 'µm Imagpart']);
    
    figure()
    mesh(abs(image_complex));
    title(['45A 400kHz ' num2str(2^var*100) 'µm Module']);
    
    figure()
    mesh(angle(image_complex));
    title(['45A 400kHz ' num2str(2^var*100) 'µm Phase']);
end
...
%     image_sub =  image_ground - image_corr;
    ...
    figure()
    mesh(image_sub);
    title(['45A 400kHz ' num2str(2^var*100) 'µm -- Subtracted Module']);
...
%     plot(image_complex,'b-'), axis('equal')
    ...
    limit = 18*mean(std(noise_mat));    %18  ////////////////////////////
...
%     end
    ...
    for col = 1:im_corr_size(2)
        for line = 1:im_corr_size(1)
            if (length_finding(line,col) > 0.1*limit) && (mark == 0)
                first_col = col;
                mark = 1;
            elseif (length_finding(line,col) > 0.1*limit) && (mark == 1)
                last_col = col;
            end
        end
    end
    ...
    figure()
    mesh(length_finding)
    title(['45A 400kHz ' num2str(2^var*100) 'µm -- Length Finding']);
    ...
    disp(['45A 400kHz ' num2str(2^var*100) 'µm'])
...
%     max_col
    ...
    disp(['Width of fissure is ' num2str(last_col - first_col) ' steps'])
    disp('*********-----------*********')
end
...
%     plot(image_complex,'b-'), axis('equal')
    ...
    limit1 = 18*mean(std(noise_mat));   %18  ///////////////////////////
    ...
    for col = 1:im_corr_size(2)
        for line = 1:im_corr_size(1)
            if (length_finding1(line,col) > 0.1*limit1) && (mark == 0)
                first_col = col;
                mark = 1;
            elseif (length_finding1(line,col) > 0.1*limit1) && (mark == 1)
                last_col = col;
            end
        end
    end
