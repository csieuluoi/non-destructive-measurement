for var = 1:3
    buf_real = imread(['2E_400kHz_' num2str(2^var*100) 'u_realpart.tif']);
    buf_imag = imread(['2E_400kHz_' num2str(2^var*100) 'u_imagpart.tif']);
...
% Modified image_complex (fit the size of image_ground):
    ...
    figure()
    mesh(abs(image_corr));
    title(['2E 400kHz ' num2str(2^var*100) 'µm -- Module before subtraction']);
...
%-------- Modify ---------
AVE_max_vec(3) = 2*(AVE_max_vec(3));
%-------------------------

figure()
plot(depth,AVE_max_vec,'ro-','LineWidth',2)
title('2E 1003 -- Module max & Depth --');

figure()
plot(depth,AVE_angle_vec,'ro-','LineWidth',2)
title('2E 1003 -- Angle max & Depth --');